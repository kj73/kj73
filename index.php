...
function displayAllGames($gamesCatalog)
{
    ...
    foreach ($gamesCatalog as $game) {
        ...
        $rating = $game['rating'];
        $imagePath = $game['img_path']; 

        echo "<a href=\"gameDetails.php?title=" . urlencode($title) . "&genre=" . urlencode($genre) . "&developer=" . urlencode($developer) . "&platforms=" . urlencode($platforms) . "&rating=" . urlencode($rating) . "&image=" . urlencode($imagePath) . "\" class=\"gameLink\">";
